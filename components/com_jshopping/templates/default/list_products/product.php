...
<div class="product productitem_<?php print $product->product_id?>">
    <form name="product" method="post" action="<?php print 'cart/add' ;?>" enctype="multipart/form-data" autocomplete="off">
    ...
    <div class = "image">
        <?php if ($product->image){?>
            <div class="image_block">
                ...
                <a class="jcepopup" href="<? echo $baseurl ;?>components/com_jshopping/files/img_products/full_<?php print $product->product_name_image?>">
    ...
    <div class = "oiproduct">
        ...
        <input class="min_quantity" type="hidden" value="<?php echo $product->extra_field_1 ;?>" />

        <div class="buttons">
            <?php if ($product->buy_link){?>
                <input type="submit" class="addtocart_button btn btn-primary button" <? if (count($product->extra_field) && $extra_field['value'] != 1) echo 'disabled'; ?> value="<?php print _JSHOP_ADD_TO_CART?>" onclick="jQuery('#to').val('cart');" />
