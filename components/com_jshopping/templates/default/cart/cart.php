...
<?php if ($countprod > 0) : ?>
    ...
    <tr>
        <th class="jshop_img_description_center" width="10%">
            <?php print _JSHOP_IMAGE ?>
        </th>
        <th class="product_name" width="29%">
            <?php print _JSHOP_ITEM ?>
        </th>    
        <th class="single_price" width="8%">
            <?php print _JSHOP_SINGLEPRICE ?>
        </th>
        <th class="quantity" width="9%">
            <?php print _JSHOP_NUMBER ?>
        </th>
        <th class="total_price" width="7%">
            <?php print _JSHOP_PRICE_TOTAL ?>
        </th>
        <th class="remove" width="7%">
    ...
    <tr class="jshop_prod_cart <?php if ($i % 2 == 0) print "even"; else print "odd"?>">
        ...
        <td class="quantity">
            ...
            <div class="data">
                <input type = "text" name = "quantity[<?php print $key_id ?>]" value = "<?php print $prod['quantity'] ?>" class = "inputbox cart_quantitybox" />
                ...
                <input class="min_quantity" type="hidden" value="<?php echo $prod['extra_fields']['1']['value'] ;?>" />
                <input class="check_prod_quantity" type="hidden" value="0" />
            </div>
        </td>
...
<div class = "jshop cart_buttons">
    <div id = "checkout">
        ...
        <?php if ($countprod>0) : ?>
            <p class="quantity_error">Количество выбраного товара не соответствует минимально допустимому!</p>
            <a href="javascript:void(0);" title = "<?php print $this->href_checkout ?>" class = "btn check_the_button">
...
<script>
    jQuery(document).ready(function(){
        jQuery('.cart_quantitybox').each( function(){
            if (this.value.match(/[^0-9]/g)) {
                this.value = this.value.replace(/[^0-9]/g, "");
            };
            var qVal = parseInt(jQuery(this).val());
            console.log(qVal+" qVal");
            var initBlock = jQuery(this).parents(".quantity");
        ...
        jQuery('.cart_quantitybox').bind("keyup", changeQuantityCart);
        jQuery('.cart_quantitybox').bind("change", changeQuantityCart);
        jQuery('.check_the_button').click(function(){
            var cartStep2 = jQuery(this).attr('title');
            var checkButtonSum = 0; 
            jQuery('.jshop_prod_cart').each( function(){
                var checkButtonStep = parseInt(jQuery('.check_prod_quantity', this).val());
                checkButtonSum = checkButtonSum + checkButtonStep;
            });
            if ( checkButtonSum > 0 ) {
                jQuery('.quantity_error').show().delay('5000').fadeOut();
            } else {
                window.location.href = cartStep2;
            }
        });
